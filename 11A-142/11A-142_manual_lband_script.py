...
cont_spws = "3~7"
flagdata(vis=myvis, mode='manual', spw=f'{cont_spws}:0~5', flagbackup=False)
flagdata(vis=myvis, mode='manual', spw=f'{cont_spws}:58~63', flagbackup=False)

# 3 line SPWs with 2048 channels with 3.906 kHz width
line_spws = "2,8,9"
for this_spw in line_spws.split(","):
       flagdata(vis=myvis, mode='manual', spw=f'{this_spw}:0~100', flagbackup=False)
       flagdata(vis=myvis, mode='manual', spw=f'{this_spw}:1948~2047', flagbackup=False)
...
# Apply rflag to the continuum science data
flagdata(vis=myvis, flagbackup=False, mode='rflag', spw=cont_spws,
         datacolumn='CORRECTED', freqdevscale=2.5, timedevscale=3.5)
flagmanager(vis=myvis, mode='save', versionname='rflag')
